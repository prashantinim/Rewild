...
// WelcomeView presented when the app launches
struct WelcomeView: View {
    var body: some View {
        NavigationView {
            VStack(spacing: 20) {
                Text("Welcome to Rewild")
                    .font(.largeTitle)
                    .padding()
...
struct UserLocationView: View {
    ...
    @State private var userPostcode: String = ""
    @ObservedObject var viewModel = PlantViewModel()
    ...
    var body: some View {
        Form {
            Section(header: Text("Your Location")) {
                ...
                TextField("Postcode", text: $userPostcode)
            }
            ...
            Section(header: Text("Select Your Preferences")) {
                Picker("Plant Size", selection: $preferredPlantSize) {
                    ForEach(viewModel.plantSizes, id: \.self) { Text($0) }
                }

                Picker("Flowering Color", selection: $preferredFloweringColor) {
                    ForEach(viewModel.floweringColors, id: \.self) { Text($0) }
                }

                Picker("Plant Type", selection: $preferredPlantType) {
                    ForEach(viewModel.plantTypes, id: \.self) { Text($0) }
                }

                Picker("Plant Height", selection: $preferredPlantHeight) {
                    ForEach(viewModel.plantHeights, id: \.self) { Text($0) }
                }
            }

            Button("Find Plants") {
                viewModel.filterPlants(state: userState, plantSize: preferredPlantSize, floweringColor: preferredFloweringColor, plantHeight: preferredPlantHeight, plantType: preferredPlantType)
                showResults = true
            }
...
struct ResultsView: View {
    @ObservedObject var viewModel: PlantViewModel
    @State private var selectedPlant: Choice? // Assuming Choice is a well-defined type

    var body: some View {
        List(viewModel.recommendedPlants, id: \.id) { (plant: Choice) in
            NavigationLink(destination: SummaryView(selectedPlant: plant, viewModel: viewModel, wikipediaURL: URL(string: "https://en.wikipedia.org/wiki/\(plant.scientificName.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? "")")!)) {
                VStack(alignment: .leading) {
...
// Ensure there are no other top-level executable lines of code outside this struct.
